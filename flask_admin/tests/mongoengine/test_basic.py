...
def fill_db(Model1, Model2):
    ...
    Model2(
        string_field="string_field_val_1",
        int_field=None,
        float_field=None,
        bool_field=True,
    ).save()
    Model2(
        string_field="string_field_val_2",
        int_field=None,
        float_field=None,
        bool_field=False,
    ).save()
    Model2(string_field="string_field_val_3", int_field=5000, float_field=25.9).save()
    Model2(string_field="string_field_val_4", int_field=9000, float_field=75.5).save()
    Model2(string_field="string_field_val_5", int_field=6169453081680413441).save()

    Model1(test1="datetime_obj1", datetime_field=datetime(2014, 4, 3, 1, 9, 0)).save()
...
def test_column_filters(app, db, admin):
    ...
    assert [(f["index"], f["operation"]) for f in view1._filter_groups["Test1"]] == [
        (0, "contains"),
        (1, "not contains"),
        (2, "equals"),
        (3, "not equal"),
...
def test_ajax_fk(app, db, admin):
    ...
    with app.test_request_context("/admin/view/"):
        ...
        assert (
            f'data-json="[&quot;{as_unicode(model.id)}&quot;, &quot;first&quot;]"'
            in form.model1()
