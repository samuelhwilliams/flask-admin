...
class ModelView(BaseModelView):
    ...
    """
        ...
        This field accepts dictionary, where key is field name and value is either
        dictionary or instance of the
        ...
        In this example, `Post` model has child `Comment` subdocument. When generating
        form for `Comment` embedded
    ...
    def scaffold_list_columns(self):
        ...
        for n, f in self._get_model_fields():
            if isinstance(f, mongoengine.ListField) and isinstance(
    ...
    def get_list(
        ...
        # Sorting
        if sort_column:
            query = query.order_by("{}{}".format("-" if sort_desc else "", sort_column))
        else:
            order = self._get_default_order()

            if order:
                keys = [
                    "{}{}".format("-" if desc else "", col) for (col, desc) in order
                ]
